...
readonly DATA_URL='https://linqs-data.soe.ucsc.edu/public/psl-examples-data/familial-er/nih_fold_01.zip'
readonly DATA_FILE='nih_fold_01.zip'
readonly DATA_DIR='nih_fold_01'
...
function check_requirements() {
   ...
   type unzip > /dev/null 2> /dev/null
   if [[ "$?" -ne 0 ]]; then
      echo 'ERROR: unzip required to extract dataset'
      exit 11
   fi
}
